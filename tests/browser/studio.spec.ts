...
async function drawRectangle(page: Page) {
  ...
  await page.mouse.down(); await page.mouse.move(box.x + box.width * .68, box.y + box.height * .68, { steps: 16 }); await page.mouse.up(); await manualForm(page);
}
test('draw, create, dimension, undo, export and reopen', async ({ page }, testInfo) => {
  ...
  const saved = page.waitForEvent('download'); await page.getByRole('button', { name: 'Download session', exact: true }).click();
  const download = await saved; const savedPath = testInfo.outputPath('study.json'); await download.saveAs(savedPath);
  await page.getByRole('button', { name: 'New empty session' }).click();
  await page.locator('input[type=file]').first().setInputFiles(savedPath); await page.getByRole('button', {name:'Building mass 1 Building mass',exact:true}).click(); await expect(page.getByRole('spinbutton', { name: 'Height', exact: true })).toHaveValue('12');
  await page.getByRole('button', { name: 'Project', exact: true }).click();
...
test('an AI response cannot overwrite a newer manual edit', async ({ page }) => {
  ...
  await page.goto('/'); await expect(page.getByTestId('model-canvas')).toBeVisible(); await example(page); await page.getByRole('button', { name: 'Connect API', exact: true }).click();
...
test('cancelled pen strokes are discarded and the next stroke works', async ({ page }) => {
  await page.goto('/'); await expect(page.getByTestId('model-canvas')).toBeVisible(); await page.getByRole('button', { name: 'New empty session' }).click();
  await page.getByRole('button', { name: '2D pad', exact: true }).click();
  ...
  await manualForm(page); await expect(page.getByRole('button', { name: 'Create form', exact: true })).toBeDisabled();
  await drawRectangle(page); await expect(page.getByRole('button', { name: 'Create form', exact: true })).toBeEnabled();
});
test('session keys are cleared on refresh, excluded from exports, and AI requires apply', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Project', exact: true }).click();
  const downloadEvent = page.waitForEvent('download'); await page.getByRole('button', { name: 'Download session', exact: true }).click();
...
test('tablet pen input and responsive phone layout', async ({ page }, testInfo) => {
  ...
  await manualForm(page); await expect(page.getByRole('button', { name: 'Create form', exact: true })).toBeEnabled(); await page.getByRole('button', { name: 'Create form', exact: true }).click();
