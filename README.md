...
**IFM status:** IFM advertises API access at https://ifm.ai/k2/, but its hosted endpoint, model names, and account capabilities could not be verified during this build. No endpoint or model is fabricated as a working preset. Enter the values supplied by your provider. The integration is tested with simulated responses; a real IFM account/key is needed for live verification. Text models with long reasoning may need a different token budget than this POC's 4,096-token interpretation cap.
