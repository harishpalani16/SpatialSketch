...
import { distance, uid, type ModelObject, type Point, type SketchFrame, type Stroke } from '@/lib/model';
...
export type SpatialMode = 'draw' | 'navigate' | 'pick-face';
...
type Props = { objects: ModelObject[]; committedIds: string[]; strokes: Stroke[]; selectedId: string | null; onSelect: (id: string) => void; activeFrame: SketchFrame; mode: SpatialMode; tool: SketchTool; snap: boolean; touchDraw: boolean; showPlane: boolean; onFrame: (frame: SketchFrame) => void; onStroke: (s: Stroke) => void; onMessage: (message: string) => void };
type Engine = { scene: THREE.Scene; renderer: THREE.WebGLRenderer; camera: THREE.PerspectiveCamera; controls: OrbitControls; root: THREE.Group; ink: THREE.Group; guide: THREE.Group; draft: THREE.Group; dirty: boolean };

function fit(engine: Engine, direction?: 'iso' | 'top') {
  const box = new THREE.Box3().setFromObject(engine.root).union(new THREE.Box3().setFromObject(engine.ink));
...
function planeGuide(frame: SketchFrame) {
  const group = new THREE.Group();
  const plane = new THREE.Mesh(new THREE.PlaneGeometry(24, 24), new THREE.MeshBasicMaterial({ color: '#b9cba3', side: THREE.DoubleSide, transparent: true, opacity: 0.07, depthWrite: false }));
  const points: THREE.Vector3[] = [];
  for (let i = -12; i <= 12; i++) points.push(new THREE.Vector3(i,-12,0),new THREE.Vector3(i,12,0),new THREE.Vector3(-12,i,0),new THREE.Vector3(12,i,0));
  ...
  const border = new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints([[-12,-12],[12,-12],[12,12],[-12,12]].map(([x,y]) => new THREE.Vector3(x,y,0))), new THREE.LineBasicMaterial({ color: '#79936b', transparent: true, opacity: 0.6, depthWrite: false }));
...
const ModelView = forwardRef<ModelViewHandle, Props>(function ModelView(props, ref) {
  ...
  useEffect(() => {
    ...
    const engine: Engine = { scene, renderer, camera, controls, root, ink, guide, draft, dirty: true }; engineRef.current = engine;
    ...
    const rayAt = (event: PointerEvent) => { const r = canvas.getBoundingClientRect(), ray = new THREE.Raycaster(); camera.updateMatrixWorld(); engine.root.updateMatrixWorld(true); ray.setFromCamera(new THREE.Vector2((event.clientX-r.left)/r.width*2-1, -(event.clientY-r.top)/r.height*2+1), camera); return ray; };
    ...
    const pointerDown = (event: PointerEvent) => {
      ...
      if (p.mode === 'pick-face' && event.button === 0) {
        ...
        const hit = rayAt(event).intersectObjects(engine.root.children,true).find(h => h.object instanceof THREE.Mesh && p.committedIds.includes(h.object.userData.objectId));
        ...
        frame.hostObjectId = hit.object.userData.objectId; p.onFrame(frame); return;
      }
    ...
    const finish = (event: PointerEvent, cancel = false) => {
      ...
      if (!cancel && down?.id === event.pointerId && propsRef.current.mode === 'navigate' && Math.hypot(event.clientX-down.x,event.clientY-down.y) < 5) {
        const hit = rayAt(event).intersectObjects(engine.root.children,true).find(h => h.object instanceof THREE.Mesh);
        if (hit?.object.userData.objectId) propsRef.current.onSelect(hit.object.userData.objectId);
      }
      down = null;
    };
    const pointerUp = (e: PointerEvent) => finish(e), pointerCancel = (e: PointerEvent) => finish(e,true), lostCapture = (e: PointerEvent) => { if (gesture?.id === e.pointerId) finish(e,true); };
    ...
    let animation = 0;
    const render = () => { animation = requestAnimationFrame(render); if (!gesture) controls.update(); if (engine.dirty && element.clientWidth && element.clientHeight && document.visibilityState === 'visible') { renderer.render(scene,camera); engine.dirty = false; } }; render();
    return () => {
      ...
      disposeGroup(scene); renderer.dispose(); renderer.forceContextLoss(); canvas.remove(); engineRef.current = null;
    };
  }, []);
  useEffect(() => {
    const e = engineRef.current; if (!e) return;
  ...
  useEffect(() => { const e = engineRef.current; if (!e) return; disposeGroup(e.ink); e.ink.clear(); props.strokes.forEach(s => e.ink.add(strokeLine(s.points,s.plane,s.offset,s.frame))); e.dirty = true; }, [props.strokes]);
  useEffect(() => { const e = engineRef.current; if (!e) return; disposeGroup(e.guide); e.guide.clear(); if (props.showPlane) e.guide.add(planeGuide(props.activeFrame)); e.renderer.domElement.style.cursor = props.mode === 'navigate' ? 'grab' : 'crosshair'; e.dirty = true; }, [props.activeFrame,props.showPlane,props.mode]);
