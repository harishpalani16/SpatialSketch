import { z } from 'zod';
import { bounds, createObject, distance, kindSchema, objectSchema, frameSchema, pointSchema, parseProject, uid, type Project, type SketchFrame } from './model';
import { baseFrame, planeMatrix } from './spatial';
import { matrixFrame } from './session';
import { refinePoints } from './refine';

const measure = z.number().finite().min(0.001).max(200);
...
export const actionSchema = z.discriminatedUnion('type', [
  z.object({ type:z.literal('component'), kind:z.enum(['mass','slab','wall']), name:z.string().min(1).max(80), profile:z.array(pointSchema).min(2).max(128), height:objectSchema.shape.height, thickness:objectSchema.shape.thickness.optional(), offset:displacement.optional(), color:objectSchema.shape.color.optional(), frameStrokeId:z.string().max(80).optional(), frame:frameSchema.optional(), sourceStrokeIds:z.array(z.string().max(80)).max(1000).optional() }).strict(),
...
export function applyIntent(project: Project, intent: Intent, activeFrame: SketchFrame = baseFrame()): Project {
  ...
  for (const action of intent.actions) {
    if (action.type === 'component') {
      if(action.frame && action.frameStrokeId)throw new Error('Choose either a source sketch plane or an explicit component frame.');
      const source=action.frameStrokeId?result.strokes.find(s=>s.id===action.frameStrokeId):undefined;
      if(action.frameStrokeId&&!source)throw new Error('The component refers to a missing sketch plane.');
      const sourceStrokeIds=[...new Set([...(action.sourceStrokeIds||[]),...(source?[source.id]:[])])];
      if(sourceStrokeIds.some(id=>!result.strokes.some(s=>s.id===id)))throw new Error('The component refers to missing source ink.');
      const frame=source?matrixFrame(planeMatrix(source.plane,source.offset,source.frame),source.frame?.label):action.frame||activeFrame;
      const profile=action.profile.map(p=>({...p}));
      if(action.kind!=='wall'&&profile.length>2&&distance(profile[0],profile.at(-1)!)<.001)profile.pop();
      result.objects.push(objectSchema.parse({id:uid(),name:action.name,kind:action.kind,plane:'custom',frame,profile,height:action.height,thickness:action.thickness??.05,offset:action.offset??0,floors:1,color:action.color??'#c3c9aa',sourceStrokeIds}));
...
DESIGN INTERPRETATION: A sketch is loose design evidence, often multiple overlapping strokes on different planes, not a finished polygon. For requests such as 'make a facade edge detail' or 'build this idea', interpret ALL strokes in sketchContext.focusStrokeIds together. Infer coherent parts, align them with the supplied frames and dimensions, and keep the original ink unchanged. Do not refine or directly extrude raw ink by default. Use component for clean inferred parts; use primitives for standard furniture/assemblies. Never require the designer to redraw crossing ink as a simple outline just to propose an idea. Disclose which parts, dimensions, material intent and uncertain connections you inferred. These are editable conceptual parts, not verified fabrication details.
TOOLS (use exactly these fields; optional fields may be omitted):
0. {"type":"component","kind":"slab","name":"Facade return panel","frameStrokeId":"EXISTING STROKE ID","sourceStrokeIds":["EXISTING STROKE ID"],"profile":[{"x":0,"y":0},{"x":2,"y":0},{"x":2,"y":3},{"x":0,"y":3}],"height":0.025,"offset":0,"color":"#c3c9aa"}
   Author a NEW clean profile inferred from the whole spatial sketch; NEVER copy the noisy pen trail verbatim. kind slab or mass forms a solid panel with height as thickness along frame normal; wall forms a member along an OPEN clean centerline, with height along normal and thickness across its path. Profile coordinates are in the chosen frame U/V, 2..128 points, simple/noncrossing, no repeated closing point, no zero-length edges. Panel boundary points must follow perimeter order, not a zigzag across diagonals. Concave boundaries are supported. Make separate named components for panels, returns, trim, supports, etc. This tool does not modify source ink. sourceStrokeIds records the ink you interpreted.
   frameStrokeId uses that stroke's exact frozen plane including offset; choose each component's relevant stroke plane to retain 3D relationships. Omitting it uses activeFrame. Alternatively provide frame:{origin:[x,y,z],u:[x,y,z],v:[x,y,z]} with perpendicular UNIT axes in WORLD coordinates; never supply both frame and frameStrokeId. height .001..200 m, thickness .001..10 m, offset -200..200 m along normal. For an edge detail, propose a small assembly of distinct panels/returns/trim based on visible sketch relationships, not a single tall mass. Dimensions follow measured sketch extents unless the instruction requests another scale.
1. {"type":"refine","target":"stroke","id":"EXISTING ID","mode":"orthogonal"}
   target is stroke or object. straighten simplifies noisy lines to straight edges while preserving angles. orthogonal also squares corners in the outline's dominant orientation. Both preserve concave notches and shape. Only when the user explicitly requests editing ink ('refine the lines', 'make straight', 'clean up my sketch'), prefer the active stroke; for an explicitly selected building outline use object. NEVER replace a concave outline with its bounding box to straighten it. Use this tool, do not claim refinement is unsupported.
2. {"type":"create","strokeId":"EXISTING ID","kind":"mass","name":"Name","height":3.6,"thickness":0.25,"floors":1}
   Use ONLY for an explicit request to extrude/use an exact clean outline. Builds from exact sketch coordinates and frozen 3D plane. Valid kinds: mass, wall, slab, box, cylinder, sphere, gable, table, chair, pavilion. mass/slab preserve closed simple outline including concavities. wall uses open path. box/table/chair/pavilion/gable use rectangular bounds. cylinder/sphere use elliptical bounds. refine can precede create on the SAME stroke in one proposal to build from cleaned lines.
